...
export class RoleService {
  static async findAll(params: MBaseListPayload): Promise<RoleListPaginationVM> {
    ...
    const data = await RepositoryService.role.find({} , {skip: +params.skip, limit: +params.take || 10}).lean();

    response.data = data;
    response.skip = +params.skip || 0;
    response.take = +params.take || 10;
    response.total = await RepositoryService.role.model.countDocuments();
    return response;
  }
  ...
  static async delete(id: string) {
    try {
      const data = await RepositoryService.role.updateAndReturnUpdatedDocument(id, { isActive: false }).lean();
      return data;
    } catch (e) {
      throw new BadRequestException(e);
    }
